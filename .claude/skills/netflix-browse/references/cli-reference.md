...
unchanged. Naming the profile that is already active costs nothing extra;
naming a different one costs two more requests — the profile list, then the
switch — so omit it when the active profile is already the right one.

A command that takes no operand refuses one rather than ignoring it, so a
mistyped subcommand (`mylist ad 123`) is an error naming the real subcommands,
not a silent listing.
...
| `netflix version` | Version, commit and build date. Makes no request. |
| `netflix help` | The command table. Makes no request. |
...
| `netflix history [--limit N] [--csv]` | Viewing activity, newest first, dates as `YYYY-MM-DD`. `--csv` writes `date,title` to stdout and takes precedence over `--json`. |
