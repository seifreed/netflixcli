...
Answer questions about what is on Netflix **for this account, in this account's region**, and
manage the active profile's lists.

## The binary

Every example below says `netflix`. That is only a command if the binary is on `PATH`, which it is
not by default. Check first, and fall back to the copy in the checkout:

```sh
command -v netflix || echo "$NETFLIXCLI_DIR/netflix"
```

If it is not built yet, build it from the repository root — `make build` writes `./netflix` beside
the Makefile:

```sh
cd "$NETFLIXCLI_DIR" && make build
```

Set `NETFLIXCLI_DIR` to wherever this repository is checked out, or install the binary once and
forget about it:

```sh
go install github.com/seifreed/netflixcli/cmd/netflix@latest   # puts `netflix` on PATH
```

`netflix version` prints the version, the commit and the build date, and is the cheapest way to
confirm you are calling the binary you think you are — it makes no network request.
...
| `open`, `version`, `help` | 0 — no request at all |
| `--profile` naming the profile already active | +0 |
| `--profile` naming a different one | +2 (the profile list, then the switch) |
...
It is not the same everywhere, because the natural answer is not the same:

| Command | Without `--limit` | With `--limit N` |
|---|---|---|
| `search` | the first page, 48 titles | N titles, paging as needed |
| `mylist`, `continue`, `liked`, `reminders` | the **whole** list | the first N |
| `episodes` | the **whole** season | the first N |
| `browse`, `top` | whatever the row carries (13) | N per row |

So `--limit` only ever *reduces* what you get, except for `search`, where raising
it past 48 fetches more.
...
| `takes no arguments — did you mean …` | you mistyped a subcommand: `mylist ad` for `mylist add`. The hint names the real ones. |
| `netflix: command not found` | the binary is not on `PATH` — see **The binary** above. |
